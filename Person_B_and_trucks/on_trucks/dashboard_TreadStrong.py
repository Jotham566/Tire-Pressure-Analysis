...
def main():
    st.set_page_config(
        page_title="Tire Sound Data Dashboard (TREAD - STRONG: Person A's Hit)",
        page_icon="🚗",
        layout="wide",
    ...
    st.markdown("<h1 style='text-align: center; color: #1f77b4;'><i class='fas fa-tachometer-alt'></i> Tire Sound Data Dashboard (TREAD - STRONG: Person A's Hit)</h1>", unsafe_allow_html=True)
    ...
    processed_dir = Path.cwd() / 'Processed_Mounted(abcd@10_TREADStrong-A)'
    processed_trimmed_dir = Path.cwd() / 'Processed_Mounted_Trimmed'

    if data_type == "Truck-Mounted":
        data_dir = Path.cwd() / 'Processed_Mounted(abcd@10_TREADStrong-A)'
