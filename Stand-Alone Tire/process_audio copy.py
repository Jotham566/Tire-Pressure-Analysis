...
def parse_tire_info(full_segment_filename):
    """
    Extract tire number and pressure from the full segment filename.
    ...
    Raises:
        ValueError: If tire number or pressure cannot be extracted.
    """
    ...
    # Search for known pressure in the remaining_info using regex
    pressure_found = None
    ...
    return tire_num, pressure_found
...
def save_to_csv(tire_num, pressure, segments, output_dir):
    """
    ...
    for idx, seg in enumerate(segments, start=1):
        row = [f"signal segment {idx}"] + seg.tolist()
        data.append(row)

    # Define column names
    columns = ["Segment ID / Value index"] + [f"Signal Value {i}" for i in range(1, DESIRED_LENGTH + 1)]
    df = pd.DataFrame(data, columns=columns)

    # Define CSV filename
    filename = f"{tire_num}_{pressure}.csv"
...
def main():
    ...
    for full_segment_filename in tqdm(full_segment_files, desc="Processing Full Segments"):
        try:
            ...
            tire_num, pressure = parse_tire_info(full_segment_filename)
            logging.info(f"Processing file: {full_segment_filename} (Tire: {tire_num}, Pressure: {pressure})")
        ...
        # Save to CSV
        try:
            save_to_csv(tire_num, pressure, processed_hits, OUTPUT_FOLDER)
