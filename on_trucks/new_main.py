...
class TireSoundProcessor:
    ...
    def calculate_dynamic_noise_threshold(self, segment_values: pd.Series) -> float:
        ...
        """
        try:
            # Get baseline from initial window (increased window size)
            baseline = segment_values.iloc[:20]  # Increased from 10
            ...
            signal_mean = segment_values.mean()
            
            # Calculate thresholds with more conservative parameters
            statistical_threshold = baseline_mean + (5.0 * baseline_std)  # Increased multiplier
            minimum_threshold = signal_range * 0.05  # Increased to 5%
            
            # Add validation checks
            if statistical_threshold < baseline_mean:
                statistical_threshold = baseline_mean + (signal_range * 0.1)  # 10% of range
...
if __name__ == "__main__":
    ...
    STANDALONE_DIRECTORY = 'Stand Alone Data'
    OUTPUT_DIRECTORY = '.'
    ...
    try:
        ...
        noise_threshold = config.get('new_main', {}).get('noise_threshold', 0.03)
        ...
        baseline_window_size = config.get('new_main', {}).get('baseline_window_size', 10)
        std_dev_multiplier = config.get('new_main', {}).get('std_dev_multiplier', 3.0)
        min_threshold_percentage = config.get('new_main', {}).get('min_threshold_percentage', 0.01)
